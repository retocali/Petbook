function Data() {
  ...
  this.dogs = [
    ...
    },
    {
      ...
      "status": "<3 <3 <3",
      "bio": "woof <3",
      "profilePics" : [ 'profiles/profile-posts/Richard1.jpg',
                        'profiles/profile-posts/Richard2.jpg',
                        'profiles/profile-posts/Richard3.jpg',
                        'profiles/profile-posts/Richard4.jpg',
                        'profiles/profile-posts/Richard5.jpg',
                        'profiles/profile-posts/Richard6.jpg',
                        'profiles/profile-posts/Richard7.jpg',
                        'profiles/profile-posts/Richard8.jpg',
                        'profiles/profile-posts/Richard9.jpg',
                        'profiles/profile-posts/Richard10.jpg',
...
class State {
  ...
  static addMyPets(data) {
    ...
    static favoritePet(petID) {
      ...
      if (!result) {
        sessionStorage.setItem("favorites", JSON.stringify([petID]));
        return;
      }
      result = JSON.parse(result);
      if (result.indexOf(petID) == -1) {
        result.push(petID)
        sessionStorage.setItem("favorites", JSON.stringify(result));
      }
    }
    ...
    static adoptPet(petID) {
      let result = sessionStorage.getItem("adopted_pets");
      if (!result) {
        sessionStorage.setItem("adopted_pets", JSON.stringify([petID]));
        return;
      }
      result = JSON.parse(result);
      if (result.indexOf(petID) == -1) {
        result.push(petID)
        sessionStorage.setItem("adopted_pets", JSON.stringify(result));
      }
    }
    ...
    static addUncheckedType(type) {
      let result = sessionStorage.getItem("unchecked");
      if (!result) {
        sessionStorage.setItem("unchecked", JSON.stringify([type]));
        return;
      }
      result = JSON.parse(result);
      if (result.indexOf(type) == -1) {
        result.push(type)
        sessionStorage.setItem("unchecked", JSON.stringify(result));
      }
    }

    static removeUncheckedType(type) {
      let result = sessionStorage.getItem("unchecked");
      if (!result) {
        return;
      }
