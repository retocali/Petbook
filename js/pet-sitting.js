...
Util.events(document, {
	...
	"DOMContentLoaded": function() {
        ...
        for (let i = 1; i <= num_buttons; i++) {
            ...
            let result = sessionStorage.getItem("emailed-pet-setting-btn"+i);
            console.log(result);
            if (result) {
        ...
        Util.one("#yesbtn").addEventListener('click', function(e) {
            ...
            if(buttonObj.innerHTML == "Email User"){
                ...
                sessionStorage.setItem("emailed-pet-setting-"+button, true);
            }
            else{
                buttonObj.innerHTML = "Email User";
                sessionStorage.setItem("emailed-pet-setting-"+button, false);
            }
...
function buttonPress(){
    let buttonObj = document.getElementById(button);
    if(buttonObj.innerHTML == "Email User"){
        document.getElementById("warning").innerHTML = "Send an email asking for a pet-sitting service from this user?"
    }
    else{
